...
RC JoinPushdownRewriter::rewrite(std::unique_ptr<LogicalOperator> &oper, bool &change_made)
{
  ...
  std::unique_ptr<LogicalOperator>& right_child = oper->children()[0];
  ...
  rc = get_exprs_can_pushdown(join_oper_exprs, pushdown_exprs);
...
{
  RC rc = RC::SUCCESS;
  for (std::unique_ptr<Expression>& expr : exprs) {
    // 将比较的表达式下推
