...
RC IndexMeta::init(const char *name, const std::vector<FieldMeta> &fields, bool is_unique)
{
  ...
  std::transform(fields.begin(), fields.end(), std::back_inserter(fields_), [](auto &&field) { return field.name(); });
  return RC::SUCCESS;
}
...
{
  const Json::Value &is_unique = json_value[UNIQUE_FLAG];
  const Json::Value &name_value = json_value[FIELD_NAME];
  const Json::Value &field_value = json_value[FIELD_FIELD_NAME];
  if (!is_unique.isBool()) {
    LOG_ERROR("Unique of index [%s] is not boolean.",
  ...
  for (int i = 0; i < field_value.size(); ++i) {
    if (!field_value[i].isString()) {
      LOG_ERROR("Field name of index [%s] is not a string. json value=%s",
        name_value.asCString(),
        field_value[i].toStyledString().c_str());
      return RC::INTERNAL;
    }
...
const std::vector<std::string> &IndexMeta::field() const { return fields_; }
